...
IF (ierval /= 0) THEN
  WRITE(errstr,'(A,I8,A)')'cannot allocate ',this%cursize,' words'
  CALL raise_error(errstr, ierval, ier)
  IF (PRESENT(size)) THEN
    CALL rawgrib_allocate(this)
  ENDIF
ENDIF
...
IF (ierval > 0) THEN
  CALL raise_error('Error, from routine gribex, code ',ierval, ier)
  RETURN
ENDIF
ALLOCATE(this%zsec4(MAX(1,this%isec4(1))), STAT=ierval)
IF (ierval /= 0) THEN
  WRITE(errstr,'(A,I8,A)')'Error, cannot allocate ',this%isec4(1),' words'
  CALL raise_error(errstr, ierval, ier)
  RETURN
ENDIF
