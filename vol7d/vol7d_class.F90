...
if (present(description))then
  ...
  call getarg(0,arg)
  ldescription="Vol7d generated by: "//arg
end if
...
inquire(unit=lunit,opened=opened,EXIST=exist)
if (.not. opened .and. exist)CALL raise_error('unit not specified and file exist; cannot open new file')
if (.not. opened .and. .not.exist) open (unit=lunit,file=arg//".v7d",form="UNFORMATTED")
...
if (associated(this%level))     read(unit=unit)this%level
...
if (associated(this%voldatir))     read(unit=unit)this%voldatir
if (associated(this%voldatiattrr)) read(unit=unit)this%voldatiattrr

if (associated(this%volanai))      read(unit=unit)this%volanai
if (associated(this%volanaattri))  read(unit=unit)this%volanaattri
if (associated(this%voldatii))     read(unit=unit)this%voldatii
if (associated(this%voldatiattri)) read(unit=unit)this%voldatiattri

if (associated(this%volanab))      read(unit=unit)this%volanab
if (associated(this%volanaattrb))  read(unit=unit)this%volanaattrb
if (associated(this%voldatib))     read(unit=unit)this%voldatib
if (associated(this%voldatiattrb)) read(unit=unit)this%voldatiattrb

if (associated(this%volanad))      read(unit=unit)this%volanad
if (associated(this%volanaattrd))  read(unit=unit)this%volanaattrd
if (associated(this%voldatid))     read(unit=unit)this%voldatid
if (associated(this%voldatiattrd)) read(unit=unit)this%voldatiattrd

if (associated(this%volanac))      read(unit=unit)this%volanac
