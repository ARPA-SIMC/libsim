...
do iiiii=1,ndativar/**/VOL7D_POLY_TYPES_V
  ...
  if (lattr_only) then
    ...
    ier=idba_set (this%handle,"*context_id",this%data_id(i,ii,iii,iiii,iiiiii))
    ier=idba_set (this%handle,"*var_related",this%vol7d%dativar% VOL7D_POLY_TYPES_V(iiiii)%btable )
    ...
    ier=idba_set (this%handle,this%vol7d%dativar% VOL7D_POLY_TYPES_V(iiiii)%btable , &
     this%vol7d%voldati/**/VOL7D_POLY_TYPES_V(i,ii,iii,iiii,iiiii,iiiiii))
    write =.true.
  ...
  if (any(lattrr).or.any(lattri).or.any(lattrb).or.any(lattrd).or.any(lattrc))then
    ...
    if (write) ier=idba_prendilo (this%handle)
...
#undef VOL7D_POLY_TYPES
    ...
    ier=idba_unset (this%handle,this%vol7d%dativar% VOL7D_POLY_TYPES_V(iiiii)%btable )
