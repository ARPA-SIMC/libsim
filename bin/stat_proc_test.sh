...
R1JJQgAAbAEAADQCYoD/gD0BAAAHAxcBAAEAAAAAAAAVAAAAAAAAAAAAAAAAAAAAAQECBAEwMDAx
AAAAAAAgAP8AABAAHwDqYAAAAIAAAAAAdTAH0AfQAAAAAAAAAAwIgApBEAAAAAA3Nzc3
EOF

#grib_set -s table2Version=2,indicatorOfParameter=61,hour=1 test_$$.grib test_tp_$$.grib

for i in `seq 1 6`; do
    grib_set -s timeRangeIndicator=4,P1=0,P2=$i -d $(($i*($i+1)/2)) test_tp_$$.grib test_tp_${i}_$$.grib
done
