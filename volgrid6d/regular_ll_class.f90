...
if (associated(dim%lon)) then
  if (dim%nx /= size(dim%lon,1) .or. dim%ny /= size(dim%lon,2)) then
    call l4f_category_log(this%category,L4F_WARNING,"reallocate lon in grid_unproj_regular_ll")
...
if (associated(dim%lat)) then
  if (dim%nx /= size(dim%lat,1) .or. dim%ny /= size(dim%lat,2)) then
    call l4f_category_log(this%category,L4F_WARNING,"reallocate lat in grid_unproj_regular_ll")
