...
! 08 Jun 2005: v.0.2  Addeddata frames and lists.
...
! *   Free-standing all-at-once vector data type
!==============================================================================
MODULE FOR2R
   ...
   integer, private, parameter            :: r8 = kind(1.0)  !--ATTENTION real single precision (pat1)
   ...
   character(len=*),parameter, private    :: nachar="NA", version="1.03"  ! <==== VERSION
   ! Note--the following format determines precision of the data transfer:
...
!================================================================================
   SUBROUTINE OPEN_R_INFO_LIST(name, date)
      ...
      write(iunit,500,advance="NO")
500   format(/,'info = structure(list',/,'(')
...
!==============================================================================
   SUBROUTINE OPEN_R_VECTOR(name)
      ...
      write(iunit,500,advance="NO") comma, name, equals
500   format(/,3A, "structure(",/,"c(")
...
!==============================================================================
   SUBROUTINE WRT_R_ITEM(name, x, ix, ax, na, last)
      ...
      ! Write one element of a numeric vector
      ...
      ! Write the VALUE of the vector entry.  If nnames(level)==0, this is
      ! the first item at this level, & we omit the comma.
...
415   format(1x,"Error: Size of missing-values matrix does not match size of data", &
      ...
      write(iunit,500) comma, name, equals
500   format(/, 3A, "structure(c(")
...
530   format(/)
      ...
      write(iunit, 600) nrow, comma, ncol
...
!==============================================================================
   SUBROUTINE WRT_R_DF_COL(name, x, ix, ax, na, last, rownames, rowids)
      ! M.H. Prager,  June 2005
      ! mike.prager@noaa.gov
      !
...
420   format(1x,"Error: Column lengths do not match in wrt_r_df_col."/&
      ...
      ! If last column, check for row names and copy into char vector:
      if (lst) then
         irn = 0
         ! If rownames are given, copy into "rname" array
         if (present(rownames)) then
            irn = 1
            allocate(rname(nrow))
            rname(1:nrow) = rownames(1:nrow)
         elseif (present(rowids)) then
            irn = 1
            ! Convert row IDs to character with internal write:
            allocate(rname(nrow))
            do i = 1, nrow
               write(rname(i),'(I0)') rowids(i)
            enddo
         endif
      endif

      !...If first column in the d.f., start w/ left paren; otherwise, comma:
      if (nnames(level) == 1) then
...
514   format(3A)
      ...
      if_last: if (lst) then
         ...
         write(iunit,520) rparen, comma,".Names = c("
         !...Write column names:
         do i=1, nnames(level)
         ...
         if (irn == 0) then
            ...
            do i = 1, nrow
               write(iunit,500,advance="NO") quote, trim(rname(i)), quote
               if (i < nrow) write(iunit, 500, advance="NO") comma
            end do
            deallocate(rname)
         end if
