...
!!
!! plot triangles generated by space_utilities
INTERFACE plot_triangles
  MODULE PROCEDURE ncar_plot_triangles, ncar_plot_triangles_d
END INTERFACE
...
!> \brief  plot triangles
!! 
subroutine ncar_plot_triangles (this,co,tri,logo)

type(ncar_plot),intent(in)  :: this
type(xy),intent(in) :: co(:)
type(triangles),intent(in) :: tri
...
if (c_e(tri%nt))then
  do k=1,tri%nt
    i=k*3
    call gpl (4,(/&
     x2ncar(co(tri%ipt(i-2))%x,mn,mx),&
     x2ncar(co(tri%ipt(i-1))%x,mn,mx),&
     x2ncar(co(tri%ipt(i))%x,mn,mx),&
     x2ncar(co(tri%ipt(i-2))%x,mn,mx)&
     /),(/&
     y2ncar(co(tri%ipt(i-2))%y,mn,mx),&
     y2ncar(co(tri%ipt(i-1))%y,mn,mx),&
     y2ncar(co(tri%ipt(i))%y,mn,mx),&
     y2ncar(co(tri%ipt(i-2))%y,mn,mx)&
     /))
  end do
end if
...
end subroutine ncar_plot_triangles

!> \brief  plot triangles
!! 
subroutine ncar_plot_triangles_d (this,x,y,tri,logo)
...
end subroutine ncar_plot_triangles_d
...
if (title /= "") title="Station: "//trim(title)
